...
def test_strided_snapshot_and_owning_decode():
    ...
    frequency, first, payload, element, element_frequency, length, marker, outer = encode_series(
        series
    )
    assert (frequency, first) == (32, 24288)
    source[:] = -1
    result = decode_series(
        frequency, first, payload, element, element_frequency, length, marker, outer
    )
