...
Mirrors ``TimeSeriesEcon.jl/src/x13/x13consts.jl`` (1,058 LOC). Landed in
**M2.1** (session 49).

Contents:

* :data:`_SERIES_EXTENSIONS` — extensions parsed as :class:`~tsecon.tseries.TSeries`.
* :data:`_PROBABLY_SERIES_EXTENSIONS` — best-effort series-parsed extensions.
* :data:`_TABLE_EXTENSIONS` — multi-column equal-length tables.
* :data:`_KV_LIST_EXTENSIONS` — key/value :class:`~tsecon.workspace.Workspace` pairs.
* :data:`_HUMAN_TEXT_EXTENSIONS` — plain-string outputs (logs, summaries).
* :data:`_ORDERED_MONTH_NAMES` — twelve lowercase three-letter month abbreviations.
* :data:`_MONTHS_AND_QUARTERS` — month-name to 1-based month-of-year mapping.
* :data:`_OUTPUT_ALT_NAMES` — per-spec ``short_code -> long_name`` mapping.
* :data:`_OUTPUT_SAVE_TABLES` — per-spec ordered list of save-able output codes.
* :data:`_OUTPUT_DESCRIPTIONS` — per-spec ``short_code -> description`` mapping.
* :data:`_OUTPUT_DESCRIPTIONS_FLAT` — derived flat ``short_code -> description``
  mapping.
* :data:`_OUTPUT_UDG_DESCRIPTION` — UDG (Census-Bureau-defined diagnostic) code
  to description mapping; flat.

All constants are module-private; siblings (``_spec.py``, ``_write.py``,
``_result.py``) import them by name. The Python port uses plain ``str`` keys
where Julia used ``Symbol`` — the in-memory shape is identical, only the
literal-syntax differs (``:b3`` becomes ``"b3"``).

Future X13as version bumps may add new output extensions; when this happens
the constants are extended in the bump PR, not regenerated. Each extension is
classified into exactly one of the five extension-set categories above; a
fall-through emits the ``unknown output file`` warning mirrored from
``x13result.jl:194-198`` (lands in M2.5).
"""
...
# ---------------------------------------------------------------------------
# Output extension classifications
# ---------------------------------------------------------------------------
# Each X-13 output file has a 2-4 character extension; the extension drives
# how the file is parsed when ``load=`` requests it. The five sets below
# partition the universe of extensions X13as v1.1 b60 emits; any extension
# not in one of these sets falls through to the ``unknown output file``
# warning path (mirrors ``x13result.jl:194-198``).
#
# Mirrors ``x13consts.jl:4-47``.

_SERIES_EXTENSIONS: Final[frozenset[str]] = frozenset(
    {
        # generic
        "rrs",
        "rmx",
        ...
        "s16",
        "s18",
        # SEATS / sliding-spans / spectrum / X-11 misc
        "tad",
        "psf",
        "pir",
        "pe5",
...
_PROBABLY_SERIES_EXTENSIONS: Final[frozenset[str]] = frozenset(
    {
        "sas",
        "ais",
        "so",
...
# Untreated output codes the Julia upstream catalogued as "we know these
# exist but don't read them" — kept as a reference set so the M2.5
# `unknown output file` path can disambiguate "known but skipped" from
# "X-13 version drift introduced a genuinely new code". Mirrors
# ``x13consts.jl:18-36``.
_UNTREATED_EXTENSIONS: Final[frozenset[str]] = frozenset(
    {
        "smy",  # slidingspans
        "sis",
        "cis",
        "ais",
        "yis",  # slidingspans (ind*)
        "psa",  # seats
        "is1",
        "is2",
        "it0",
...
_TABLE_EXTENSIONS: Final[frozenset[str]] = frozenset(
    {
        "pcf",
        "acf",
        "ac2",
        ...
        "fac",
        "wkf",
        # Julia uses ``Symbol(13)`` here — the symbol whose name is the digit
        # string ``"13"``. In Python that's just the plain string ``"13"``.
        "13",
    }
)

_KV_LIST_EXTENSIONS: Final[frozenset[str]] = frozenset({"lks", "mdc"})

_HUMAN_TEXT_EXTENSIONS: Final[frozenset[str]] = frozenset(
    {
        "spc",  # the input spec file (echo of what we sent)
        "gmt",  # list of files in the graphics folder
        "out",  # the captured output of the print command
        "sum",  # SEATS summary file
    }
)


# ---------------------------------------------------------------------------
# Month / quarter name tables
# ---------------------------------------------------------------------------
# Mirrors ``x13consts.jl:49-63``. Used by ``_spec._mit_to_spc`` for Monthly
# MITs and by ``_write`` for spec-builder serialization (e.g. the ``per``
# field of an ``arima`` builder for monthly data is written as ``per=jan``,
# not ``per=1``). The Monthly-frequency .spc serialization is the only place
# the lowercase three-letter names appear in user-visible output.

_ORDERED_MONTH_NAMES: Final[tuple[str, ...]] = (
    "jan",
    "feb",
    "mar",
    "apr",
...
_MONTHS_AND_QUARTERS: Final[dict[str, int]] = {
    "jan": 1,
    "feb": 2,
    "mar": 3,
    "apr": 4,
...
# ---------------------------------------------------------------------------
# Output: per-spec short-code aliases (`_OUTPUT_ALT_NAMES`)
# ---------------------------------------------------------------------------
# When the user asks for an output via ``load=("alt_name", ...)``, the alt
# name maps to the X-13 short code that names the file on disk. Mirrors
# ``x13consts.jl:65-466``. The mapping is per-spec because the same short
# code (e.g. ``:ach``) means different things in different specs (history's
# ``aveabsrevchng`` vs automdl's ``autochoice``).

_OUTPUT_ALT_NAMES: Final[dict[str, dict[str, str]]] = {
    "arima": {},
    "automdl": {
        "ach": "autochoice",
        "amd": "autochoicemdl",
    ...
    "spectrum": {
        ...
        "is1": "specindsa",
        "tpk": "tukeypeaks",
        # Julia source repeats st1/st2 and it1/it2 keys; the last definition
        # wins under Julia's Dict literal semantics. We preserve the
        # last-write-wins outcome explicitly so behavior matches.
        "st0": "tukeyspecorig",
    ...
    "x11": {
        ...
        "c2": "trendc2",
        "c7": "trendc7",
        # Julia source has duplicate ``:tac`` keys (line 326, line 405);
        # last-write-wins makes ``"tac" -> "trendconst"`` the effective
        # mapping. Preserved here.
        "tac": "trendconst",
        "d2": "trendd2",
        "d7": "trendd7",
...
# ---------------------------------------------------------------------------
# Output: per-spec savable short-code orderings (`_OUTPUT_SAVE_TABLES`)
# ---------------------------------------------------------------------------
# Each spec exposes an ordered list of output codes that can appear in the
# ``save=`` argument. The order matters for documentation generation; the
# set membership matters for ``load=`` validation. Mirrors
# ``x13consts.jl:468-488``.

_OUTPUT_SAVE_TABLES: Final[dict[str, list[str]]] = {
    "arima": [],
    "automdl": [],
    "check": ["acf", "pcf", "ac2"],
...
# ---------------------------------------------------------------------------
# Output: per-spec short-code descriptions (`_OUTPUT_DESCRIPTIONS`)
# ---------------------------------------------------------------------------
# Human-readable descriptions for each output code, sourced from the X-13
# user manual. Used for documentation generation (the M2.6 reference page
# enumerates these into a table per spec) and for the per-attribute
# docstring of lazy result fields (M2.5). Mirrors ``x13consts.jl:490-893``.
#
# Several keys in the Julia source are duplicated within their spec dict
# (e.g. ``:nrm`` appears twice under ``:check`` and ``:seats``); Julia's
# Dict literal semantics is last-write-wins, so we preserve only the final
# value. Inline comments mark each preserved duplicate.

_OUTPUT_DESCRIPTIONS: Final[dict[str, dict[str, str]]] = {
    "arima": {},
    "automdl": {
        "ach": "model choice of automatic model procedure",
        "amd": "summary output for models estimated during choice of ARMA model orders",
        "adt": (
            "tests performed on the default model (usually the airline model) "
            "of the automatic model identification procedure"
        ),
        ...
        "adf": "choice of differencing by automatic model identification procedure",
        "mu": "choice regarding use of constant term with automatically identified model",
    },
    "check": {
        "acf": (
            "autocorrelation function of residuals with standard errors, "
            "and Ljung-Box Q-statistics computed through each lag"
        ),
        ...
        "ac2": (
            "autocorrelation function of squared residuals with standard "
            "errors, and Ljung-Box Q-statistics computed through each lag"
        ),
        "ap2": ("plot of squared residual autocorrelation function with ± 2 standard error limits"),
        # nrm: duplicate in Julia source; last-write-wins picks the "Test results
        # from the normality tests" form (line 516).
        "nrm": (
            "Test results from the normality tests on the regARIMA model "
            "residuals (Kurtosis, skewness and Geary’s a statistics)"
        ),
        # dw: duplicate; last-write-wins picks the regARIMA-model-residuals form.
        "dw": "Durbin-Watson statistic for regARIMA model residuals",
        "frt": "Friedman non-parametric test for residual seasonality",
        "hst": (
            "histogram of standardized residuals and the following summary "
            "statistics of the residuals: minimum, maximum, median, standard "
            "deviation, and robust estimate of residual standard deviation "
            "(1.48 × the median absolute deviation)"
        ),
        "lbq": "Significant lags for the Ljung-Box Q statistic",
        "bpq": "Significant lags for the Box-Pierce Q statistic",
        "sft": ("Model-based f-statistic for seasonality from Lytras, Feldpausch, and Bell (2007)"),
        "tft": "Model-based f-statistic for trading day from Pang and Monsell (2016)",
    },
    "estimate": {
        "opt": "header for the estimation options",
        "mdl": (
            "if used with the print argument, this controls printing of a "
            "short description of the model; if used with the save argument, "
            "this creates a file containing regression and arima specs "
            "corresponding to the model, with the estimation results used to "
            "specify initial values for the ARMA parameters"
        ),
        ...
        "lks": (
            "log-likelihood at final parameter estimates and, if exact = arma "
            "is used (default option), corresponding model selection criteria "
            "(AIC, AICC, Hannan-Quinn, BIC)"
        ),
        "itr": (
            "detailed output for estimation iterations, including "
            "log-likelihood values and parameters, and counts of function "
            "evaluations and iterations"
        ),
        "ite": ("error messages for estimation iterations, including failure to converge"),
        "rcm": (
            "correlation matrix of regression parameter estimates if used "
            "with the print argument; covariance matrix of same if used with "
            "the save argument"
        ),
        "acm": (
            "correlation matrix of ARMA parameter estimates if used with the "
            "print argument; covariance matrix of same if used with the save "
            "argument"
        ),
        ...
        "ref": (
            "Xβˆ, matrix of regression variables multiplied by the "
        ...
        "acc": "Akaike’s Information Criterion (AIC) adjusted for the length of the series",
        "bic": "Baysean Information Criterion (BIC)",
        "hq": "Hannan-Quinn Information Criterion",
    },
    "force": {
        "saa": (
            "final seasonally adjusted series with constrained yearly totals "
            "(if type = regress or type = denton)"
        ),
        "rnd": (
            "rounded final seasonally adjusted series (if round = yes) or the "
    ...
    "forecast": {
        ...
        "fvr": (
            "forecast error variances on the transformed scale, showing the "
            "contributions of the error assuming the model is completely "
            "known (stochastic variance) and the error due to estimating any "
            "regression parameters (error in estimating AR and MA parameters "
            "is ignored)"
        ),
        "fct": (
            "point forecasts on the original scale, along with upper and "
            "lower prediction interval limits"
        ),
        "btr": "backcasts on the transformed scale, with corresponding forecast standard errors",
        "bct": (
            "point backcasts on the original scale, along with upper and "
            "lower prediction interval limits"
        ),
    },
    "history": {
        ...
        "rot": (
            "record of outliers removed and kept for the revisions history "
            "(printed only if automatic outlier identification is used)"
        ),
        ...
        "chr": (
            "revision from concurrent to most recent estimate of the "
            "month-to-month (or quarter-to-quarter) changes in the seasonally "
            "adjusted data"
        ),
        "chs": (
            "summary statistics for revisions in the month-to-month (or "
            "quarter-to-quarter) changes in the seasonally adjusted data"
        ),
        "iar": (
            "revision from concurrent to most recent estimate of the indirect "
        ...
        "tcr": (
            "revision from concurrent to most recent estimate of the "
            "month-to-month (or quarter-to-quarter) changes in the trend "
            "component"
        ),
        "tcs": (
            "summary statistics for revisions in the month-to-month (or "
        ...
        "sfr": (
            "revision from concurrent to most recent estimate of the seasonal "
            "factor, as well as projected seasonal factors."
        ),
        ...
        "sfh": (
            "record of seasonal filter selection for each observation in the "
            "revisions history (printed only if automatic seasonal filter "
            "selection is used)"
        ),
        "sae": "concurrent and most recent estimate of the seasonally adjusted data",
        "che": (
            "concurrent and most recent estimate of the month-tomonth (or "
            "quarter-to-quarter) changes in the seasonally adjusted data"
        ),
        ...
        "sfe": (
            "concurrent and most recent estimate of the seasonal factors and "
            "projected seasonal factors"
        ),
        "fch": (
            "listing of the forecast and forecast errors used to generate "
            "accumulated sum of squared forecast errors"
        ),
        "asa": "average absolute revision of the seasonally adjusted series",
        "ach": (
            "average absolute revision of the month-to-month (or "
            "quarter-to-quarter) changes in the seasonally adjusted data"
        ),
        "iaa": "average absolute revision of the indirect seasonally adjusted series",
        "atr": "average absolute revision of the final trend component",
        "atc": (
            "average absolute revision of the month-to-month (or "
            "quarter-to-quarter) changes in the trend component"
        ),
        "asf": "average absolute revision of the final seasonal factors",
        "asp": "average absolute revision of the projected seasonal factors",
        "afe": "average sum of squared forecast error for each forecast lag",
    },
    "metadata": {},
    "identify": {
        "iac": (
            "sample autocorrelation function(s), with standard errors and "
            "Ljung-Box Q-statistics for each lag"
        ),
        "acp": (
            "line printer plot of sample autocorrelation function(s) with "
            "± 2 standard error limits shown on the plot"
        ),
        "ipc": "sample partial autocorrelation function(s) with standard errors for each lag",
        "pcp": (
            "line printer plot of sample partial autocorrelation function(s) "
            "with ± 2 standard error limits shown on the plot"
        ),
        "rgc": (
            "Regression coefficients removed from the transformed series "
            "before ACFs and PACFs were generated."
        ),
    },
    "outlier": {
        "hdr": (
            "options specified for outlier detection including critical "
            "value, outlier span, and types of outliers searched for"
        ),
        "oit": (
            "detailed results for each iteration of outlier detection "
            "including outliers detected, outliers deleted, model parameter "
            "estimates, and robust and non-robust estimates of the residual "
            "standard deviation"
        ),
        "ots": (
            "t-statistics for every time point and outlier type on each outlier detection iteration"
        ),
        "tls": "summary of t-statistics for temporary level shift tests",
        "fts": (
            "t-statistics for every time point and outlier type generated "
            "during the final outlier detection iteration"
        ),
    },
    "pickmdl": {
    ...
    "regression": {
        ...
        "ats": (
            "output from AIC-based test(s) for trading day, Easter, and "
        ...
        "ls": (
            "regARIMA level change, temporary level change and ramp outlier factors (table A8.LS)"
        ),
    ...
    "seats": {
        ...
        "smd": "Model used by the SEATS module for signal extraction",
        "xmd": "Model submitted to the SEATS module",
        "nrm": "Normality test",
        "oue": "Over-under estimation diagnostics",
        "tse": "Total mean squared error",
        "cvr": "Component variances",
        "cee": "Concurrent estimation error",
        "prs": "Percent reduction standard error",
        "aad": "Annual Average absolute difference",
        "ssg": "Test for seasonal significance",
    },
    "series": {
        "hdr": "summary of options selected for this run of X-13-ARIMA-SEATS",
        "a1": (
            "time series data, with associated dates (if the span argument is "
            "present, data are printed and/or saved only for the specified span)"
        ),
        ...
        "b1p": (
            "plot of the prior adjusted original series augmented by "
            "prior-adjusted forecasts (if specified); if no prior factors or "
            "forecasts are used, the original series is plotted"
        ),
    },
    "slidingspans": {
        "hdr": "header text for the sliding spans analysis",
        "ssf": (
            "F-tests for stable and moving seasonality estimated over each of the sliding spans"
        ),
        "fmn": "range analysis for each of the sliding spans",
        "pct": (
            "table showing the percent of observations flagged as unstable "
            "for the seasonal and/or trading day factors, final seasonally "
            "adjusted series (if necessary), and the month-to-month (or "
            "quarter-toquarter) changes"
        ),
        "sum": (
            "tables, histograms and hinge values summarizing the percentage "
            "of observations flagged for unstable seasonal and/or trading "
            "day factors, final seasonally adjusted series (if necessary), "
            "and month-tomonth (or quarter-to-quarter) changes"
        ),
        "smy": (
            "additional tables, histograms and hinge values summarizing the "
            "percentage of observations flagged for the year-to-year changes"
        ),
        "fmi": (
            "range analysis for the implicit adjustment factors of the "
            "indirectly seasonally adjusted series"
        ),
        "pci": (
            "tables of the percent of observations flagged as unstable for "
            "the seasonal factors and month-tomonth (or quarter-to-quarter) "
            "changes of the indirect seasonal adjustment"
        ),
        "smi": (
            "tables, histograms and hinge values summarizing the percentage "
            "of observations flagged for unstable seasonal factors, "
            "month-to-month (or quarter-toquarter) and year-to-year changes "
            "for the indirect adjustment"
        ),
        "pcy": (
            "additional entry for the percent of observations flagged as "
            "unstable for the year-to-year changes"
        ),
        ...
        "pyi": (
            "additonal entry for the percent of observations flagged as "
            "unstable for the year-to-year (or quarter-to-quarter) changes "
            "of the indirect seasonal adjustment"
        ),
        "syi": (
            "additional tables, histograms and hinge values summarizing the "
            "percentage of observations flagged for the year-to-year changes "
    ...
    "spectrum": {
        ...
        "sp0": "spectral plot of the first-differenced original series",
        "sp1": (
            "spectral plot of differenced, X-11 seasonally adjusted series "
            "(or of the logged seasonally adjusted series if mode = logadd "
            "or mode = mult)"
        ),
        ...
        "is1": (
            "spectral plot of outlier-modified irregular series from the "
        ...
        # st1: duplicate in Julia source (line 303 + 305); last-write-wins picks the SEATS form.
        "st1": "Tukey spectrum of the differenced final SEATS seasonal adjustment",
        # st2: duplicate in Julia source (line 304 + 306); last-write-wins picks the SEATS form.
        ...
        "it1": (
            "Tukey spectrum of the outlier-modified irregular series from "
            "the indirect seasonal adjustment"
        ),
        "dpk": "Visually significant peaks in spectra for direct seasonal adjustment",
        "dqs": "QS diagnostic to detect seasonality, direct seasonal adjustment",
        "dtp": "Peak probabilities for Tukey spectra for direct seasonal adjustment",
        "ipk": "Visually significant peaks in spectra for indirect seasonal adjustment",
        "iqs": "QS diagnostic to detect seasonality, indirect seasonal adjustment",
        "itp": "Peak probabilities for Tukey spectra for inddirect seasonal adjustment",
        "spk": "Visually significant peaks in spectra",
        "stp": "Significant peaks of Tukey spectrum",
        "a1": "original series",
        "a19": "original series, adjusted for regARIMA outliers",
        "b1": "original series, adjusted for user specified and reg-",
    ...
    "transform": {
        ...
        "acp": (
            "plot of original series with value from the constant argument added to the series"
        ),
        ...
        "a4d": (
            "prior-adjusted series (including prior trading day adjustments), with associated dates"
        ),
        "a4p": (
            "prior-adjusted series using only permanent prior factors and "
            "prior trading day adjustments, with associated dates"
        ),
    ...
    "x11": {
        ...
        "tad": (
            "total adjustment factors (only printed out if the original series "
            "contains values that are ≤ 0)"
        ),
        ...
        # tac: duplicate in Julia source (line 326 + 405); last-write-wins picks
        # the "final trend component" form.
        ...
        "ratioplotorig": (
            "month-to-month (or quarter-to-quarter) ratio plots of the original series"
        ),
        "ratioplotsa": (
            "month-to-month (or quarter-to-quarter) ratio plots of the seasonally adjusted series"
        ),
        ...
        "fb1": "F-test for stable seasonality, performed on the original series",
        "fd8": "F-test for stable seasonality, performed on the final SI-ratios",
        "icr": "I/¯ C¯ ratio",
        "ids": "Identifiable seasonality test result",
        "m1": "M1 Quality Control Statistic",
        "m10": "M10 Quality Control Statistic",
        "m11": "M11 Quality Control Statistic",
        "m2": "M2 Quality Control Statistic",
        "m3": "M3 Quality Control Statistic",
        "m4": "M4 Quality Control Statistic",
        "m5": "M5 Quality Control Statistic",
        "m6": "M6 Quality Control Statistic",
        "m7": "M7 Quality Control Statistic",
        "m8": "M8 Quality Control Statistic",
        "m9": "M9 Quality Control Statistic",
        "msf": "F-test for moving seasonality",
        "msr": "Moving seasonality ratio",
        "q": "Overall index of the quality of the seasonal adjustment",
        "q2": "Q statistic computed without the M2 Quality Control statistic",
    },
    "x11regression": {
        ...
        "xoh": (
            "options specified for outlier detection including critical value and outlier span"
        ),
        ...
        "xrc": (
            "correlation matrix of irregular regression parameter estimates "
            "if used with the print argument; covariance matrix of same if "
            "used with the save argument"
        ),
    },
}


# Derived: flat ``short_code -> description`` mapping. Built once at module
# import; the spec-name partition is recoverable from ``_OUTPUT_DESCRIPTIONS``
# directly. Mirrors ``x13consts.jl:895`` (which uses
# ``Iterators.flatten(pairs.(values(d)))``); under Python's later-key-wins
# dict-merge semantics, duplicates across specs resolve to the
# alphabetically-last spec's value. Consumers that need per-spec
# disambiguation should index ``_OUTPUT_DESCRIPTIONS`` directly.
_OUTPUT_DESCRIPTIONS_FLAT: Final[dict[str, str]] = {
    code: description
    for spec_descriptions in _OUTPUT_DESCRIPTIONS.values()
    for code, description in spec_descriptions.items()
}


# ---------------------------------------------------------------------------
# UDG file: short-code descriptions
# ---------------------------------------------------------------------------
# The X-13 UDG (User Diagnostic Group) output file is a flat key/value listing
# of diagnostics. Each short code maps to a one-line description. Mirrors
# ``x13consts.jl:897-1048``. Used by the M2.5 UDG-file reader to label fields
# of the parsed diagnostic Workspace.

_OUTPUT_UDG_DESCRIPTION: Final[dict[str, str]] = {
    ...
    "indyyss": (
        "sliding spans of the year-to-year changes in the indirect seasonally adjusted series"
    ),
